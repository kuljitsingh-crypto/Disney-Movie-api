...
from flaskserver import db,bcrypt,mail,app
...
user_conformation={} # key-value pair of system code to track which usrer has which code
movie_data=None
...
def send_verify_code(email,code):
	msg=Message("One Time Verification Code",sender=current_app.config["MAIL_USERNAME"],recipients=[email])
...
def send_resetpwd_code(email,code):
	msg=Message("Password Reset Verification Code",sender=current_app.config["MAIL_USERNAME"],recipients=[email])
...
@current_app.route("/generateapikey",methods=['GET','POST'])
def generateApiKeyPage():
	if current_user.is_authenticated:
		return redirect(url_for("homePage"))
	global user_conformation
	...
	if form.validate_on_submit():
		...
		user_conformation[f"{user.email}"]=system_generated_code
...
def login():
	...
	if current_user.is_authenticated:
		...
		if user and user.isverified and current_user.is_authenticated:
			...
			if user:
				...
				if verified_user:
					if bcrypt.check_password_hash(user.password,form.password.data):
						...
						login_user(user)
						try:
							del user_conformation[f"{user.email}"]
					...
					try:
						if form.onetime_verification_code.data.lower()==user_conformation.get(f"{user.email}","").lower():
						...
						if not user_conformation.get(f"{user.email}",False):
							return redirect_to_genapikey()
			else:
				if user_conformation.get(f"{form.user_email.data}",False):
					flash("Incorrect email or password!.", 'alert')
					form.user_email.data=None
				else:
					return redirect_to_genapikey()
	return render_template('login.html',title="Login",form=form,about_activated=True,
		                   verified_field_req= not user.isverified if user else not user_verification_status ,
		                   server=current_app,css_file=url_for('static',filename='style.css',q=time.time()))
...
@current_app.route("/disneymovies/all/",methods=['GET'])
def allMovies():
	user_apikey=request.args.get('apikey').strip()
	user=User.query.filter_by(apikey=user_apikey).first()
	if user:
		file='movie_dataset.json'
		filename=os.path.join(current_app.root_path,'static/data',file)
...
@current_app.route("/resetpassword",methods=["GET","POST"])
def resetPassword():
	global user_conformation
	status=False
	...
	if request.method=="POST":
		if request.form.get('verify_code',None):
			if user_conformation.get(f"{form.user_email.data}",False):
				...
				if user_entered_code!=user_conformation.get(f"{form.user_email.data}",False):
					flash("Please enter valid verification code!","alert")
				else:
					status=True
...
@current_app.route("/disneymovies/",methods=['GET'])
def selectedMovie():
	global movie_data
	user_apikey=request.args.get('apikey').strip()
	user=User.query.filter_by(apikey=user_apikey).first()
...
@current_app.route("/apiusage",methods=["GET","POST"])
@login_required
def userApiUsage():
...
@current_app.route("/delacc",methods=["POST"])
@login_required
def delAcc():
